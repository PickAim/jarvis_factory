...
class JCalcClassesFactory:
    @staticmethod
    def create_db_controller() -> DBController:
        user_info_collector = JDBClassesFactory.create_user_info_collector()
        jorm_collector = JDBClassesFactory.create_jorm_collector()

        user_info_changer = JDUClassesFactory.create_user_info_changer()
